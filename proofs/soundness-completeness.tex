...
\begin{enumerate}
  ...
  \item \textbf{Whitespace.} Let
  $W = \{ \texttt{" "}, \texttt{"\textbackslash t"}, \texttt{"\textbackslash r"}, \texttt{"\textbackslash n"} \}^\ast$
  (JSON whitespace). The parser $\ws$ always succeeds, consuming the longest
  prefix $w \in W$ and leaving the remaining suffix.
  \item \textbf{Lexemes.}
  \begin{enumerate}
    \item $\mathsf{symbol}(c)$ succeeds iff the input starts with character $c$,
...
\end{enumerate}
We treat $W$ as a distinguished nonterminal with the JSON whitespace language,
...
\begin{enumerate}
  ...
  $\mathsf{stringLiteral} \seq \mathsf{symbol}(\texttt{":"}) \seq
  \mathsf{jsonValue}$.
  \item $\mathsf{jsonArray}$ parses
  $\mathsf{symbol}(\texttt{"["}) \seq
  ...
  \mathsf{symbol}(\texttt{"\}"})$.
  \item $\mathsf{jsonValue}$ parses $(\mathsf{jsonNull} <|>
  \mathsf{jsonBool} <|> \mathsf{jsonNumber} <|> \mathsf{jsonString} <|>
  \mathsf{jsonArray} <|> \mathsf{jsonObject})$.
  \item The top-level parser is $\parse(\ws \seq \mathsf{jsonValue}, s)$.
\end{enumerate}

\section*{Soundness}

\paragraph{Theorem (Soundness).}
If $\parse(\ws \seq \mathsf{jsonValue}, s) = \mathrm{Just}(v)$, then
$s \in L(\mathsf{value})$.
...
By Axioms 5 and 6, $\ws$, $\mathsf{symbol}$, $\mathsf{keyword}$, $\mathsf{comma}$,
...
$\mathsf{objectField}$ consumes a $\mathsf{string}$, then
$\mathsf{symbol}(\texttt{":"})$, then a $\mathsf{value}$. By Lemma S1, Axiom 3,
and Axiom 4
...
$\mathsf{jsonArray}$ consumes $\mathsf{symbol}(\texttt{"["})$, then a
$\mathsf{sepBy}$ list of $\mathsf{value}$ separated by $\mathsf{comma}$, then
$\mathsf{symbol}(\texttt{"]"})$.
...
$\mathsf{jsonObject}$ consumes $\mathsf{symbol}(\texttt{"\{"})$, then a
$\mathsf{sepBy}$ list of $\mathsf{member}$ separated by $\mathsf{comma}$, then
$\mathsf{symbol}(\texttt{"\}"})$.
By Lemmas S1 and S2 and Axiom 4, its consumed prefix is in
$L(\mathsf{object})$.

\paragraph{Lemma S5 (value soundness).}
$\mathsf{jsonValue}$ chooses among parsers for $\mathsf{object}$,
$\mathsf{array}$, $\mathsf{string}$, $\mathsf{number}$, $\mathsf{true}$,
$\mathsf{false}$, or $\mathsf{null}$. By Lemmas S1, S3, S4 and Axiom 4 (choice),
any success yields a prefix in $L(\mathsf{value})$ with no leading whitespace.

The lemmas are mutually consistent, so by mutual induction the soundness of
$\mathsf{jsonValue}$ holds. With top-level acceptance, the theorem follows
because $\ws$ accounts for any leading $W$, and any residual $r \in W$ can be
appended to a string in $L(\mathsf{value})$ without leaving the language
because $W$ is closed under concatenation. \qed

\section*{Completeness}

\paragraph{Theorem (Completeness).}
If $s \in L(\mathsf{value})$, then there exists $v$ such that
$\parse(\ws \seq \mathsf{jsonValue}, s) = \mathrm{Just}(v)$.
...
a $\mathsf{string}$, $\mathsf{symbol}(\texttt{":"})$, and a $\mathsf{value}$. By
Lemma C1 and Axiom 4 (sequencing), and the mutual induction hypothesis that
$\mathsf{jsonValue}$ is complete for $\mathsf{value}$, $\mathsf{objectField}$
succeeds on $s$.

\paragraph{Lemma C3 (array completeness).}
If $s \in L(\mathsf{array})$, then $s$ matches $\mathsf{symbol}(\texttt{"["})$,
then a possibly empty list of $\mathsf{value}$ separated by $\mathsf{comma}$,
then $\mathsf{symbol}(\texttt{"]"})$.
...
If $s \in L(\mathsf{object})$, then $s$ matches
$\mathsf{symbol}(\texttt{"\{"})$, then a possibly empty list of
$\mathsf{member}$ separated by $\mathsf{comma}$, then
$\mathsf{symbol}(\texttt{"\}"})$.
By Lemmas C1 and C2 and Axiom 4, $\mathsf{jsonObject}$ succeeds on $s$.

\paragraph{Lemma C5 (value completeness).}
If $s \in L(\mathsf{value})$, then by the grammar $s = w t$ for some $w \in W$
followed by one of the alternatives. By Lemmas C1--C4 and Axiom 4 (choice),
$\mathsf{jsonValue}$ succeeds on $t$; left bias does not prevent success because
choice succeeds whenever at least one branch succeeds.
...
includes trailing $W$ and $\ws$ consumes the leading whitespace prefix, the
parser $\ws \seq \mathsf{jsonValue}$ consumes the entire input $s$ when
